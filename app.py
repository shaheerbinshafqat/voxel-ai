...
def generate_detailed_summary(text, tokenizer, model):
    ...
    for chunk in chunks:
        ...
        wc = len(chunk.split())
        safe_max = min(200, max(60, int(wc * 0.5)))
        safe_min = min(40, int(safe_max * 0.5))
    ...
    if len(chunk_summaries) > 1:
        ...
        if len(combined.split()) > 150:
            ...
            ids = model.generate(inputs["input_ids"], max_length=250, min_length=80,
                                 length_penalty=1.2, num_beams=4, early_stopping=True)
...
def download_audio(youtube_url):
    try:
        ...
        ydl_opts = {
            ...
            "ffmpeg_location": FFMPEG_PATH, "quiet": True, "no_warnings": True,
            "postprocessors": [{"key": "FFmpegExtractAudio", "preferredcodec": "mp3", "preferredquality": "192"}],
        }
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(youtube_url, download=True)
            return os.path.join(temp_dir, info["id"] + ".mp3"), info
...
def download_video_for_frames(youtube_url):
    try:
        ...
        ydl_opts = {
            "format": "worst[ext=mp4]/worst", "outtmpl": output_template,
            "ffmpeg_location": FFMPEG_PATH, "quiet": True, "no_warnings": True,
...
def extract_keyframes_smart(video_path, output_dir):
    """
    Render-optimized smart keyframe extraction
    """
    ...
    try:
        cmd = [
            ...
            "-vsync", "vfr",
            "-q:v", "5",
            os.path.join(scene_dir, "s_%03d.jpg"),
            "-loglevel", "error"
        ]
        ...
        scene_frames = sorted(
            glob.glob(os.path.join(scene_dir, "s_*.jpg"))
        )

        for i, f in enumerate(scene_frames[:25]):  # limit frames
            all_frames.append({
                "path": f,
    ...
    try:
        cmd = [
            ...
            "-vf", f"fps=1/{interval}",
            "-q:v", "6",
            os.path.join(interval_dir, "i_%03d.jpg"),
            "-loglevel", "error"
        ]
        ...
        interval_frames = sorted(
            glob.glob(os.path.join(interval_dir, "i_*.jpg"))
        )

        for i, f in enumerate(interval_frames[:25]):  # limit frames
            all_frames.append({
                "path": f,
                "timestamp": i * interval,
                "source": "interval"
    ...
    paths = [f["path"] for f in all_frames]

    unique_paths = set(
        remove_duplicate_frames(paths, min_size_kb=5)
    )

    final = [
        f for f in all_frames
        if f["path"] in unique_paths
    ]
...
def extract_events(segments, keywords):
    ...
    if not keywords or not keywords.strip():
        keywords = "important, conclusion, summary, step, remember, finally, key, result, therefore"
    ...
    for seg in segments:
        ...
        for kw in kw_list:
            if kw in low and kw not in matched_kws:
                matched_kws.add(kw)
                events.append({"time": format_time(seg["start"]), "type": f"Keyword: '{kw}'", "text": seg["text"].strip()})
        if "?" in seg["text"]:
            events.append({"time": format_time(seg["start"]), "type": "Question", "text": seg["text"].strip()})
    return events
...
<style>
  ...
  .hero-icon {{ font-size:2rem; background:var(--accent-grad); -webkit-background-clip:text; -webkit-text-fill-color:transparent; filter:drop-shadow(0 0 24px var(--glow)); }}
  ...
  /* Cards */
  .card {{ background:var(--card); backdrop-filter:blur(20px) saturate(1.3); border:1px solid var(--border);
    border-radius:14px; padding:1.6rem; margin-bottom:1rem; box-shadow:var(--shadow);
  ...
  .stButton>button:hover {{ transform:translateY(-2px) !important; box-shadow:0 8px 24px var(--glow) !important; }}

  /* Tabs */
  button[data-baseweb="tab"] {{ background:transparent !important; border:none !important; color:var(--subtext) !important;
    font-family:'Outfit',sans-serif !important; font-weight:500 !important; font-size:0.85rem !important;
    padding:0.35rem 0.7rem 0.7rem !important; margin-right:0.3rem; border-bottom:2px solid transparent !important; }}
  button[data-baseweb="tab"]:hover {{ color:var(--text) !important; }}
  button[data-baseweb="tab"][aria-selected="true"] {{ color:var(--text) !important; border-bottom:2px solid var(--accent) !important; font-weight:600 !important; }}
  div[data-baseweb="tab-list"] {{ border-bottom:1px solid var(--border); margin-bottom:1rem; }}

  /* TextArea */
  .stTextArea textarea {{ background:var(--input-bg) !important; border:1px solid var(--border) !important;
    border-radius:10px !important; color:var(--text) !important; font-family:'JetBrains Mono',monospace !important;
    font-size:0.8rem !important; line-height:1.8 !important; padding:1.2rem !important; }}
  .stTextArea label {{ display:none !important; }}

  /* Download button */
  .stDownloadButton>button {{ background:var(--input-bg) !important; color:var(--accent) !important;
  ...
  /* Scroll containers */
  .custom-scroll {{ height:420px; overflow-y:auto; padding-right:10px; font-size:0.86rem; line-height:1.65; }}
  .custom-scroll::-webkit-scrollbar {{ width:3px; }}
  .custom-scroll::-webkit-scrollbar-thumb {{ background:var(--border); border-radius:6px; }}
  .event-box {{ margin-bottom:8px; padding:11px 14px; background:var(--input-bg); border-radius:9px; border:1px solid var(--border); }}

  /* Frames */
  .frame-card {{ border-radius:9px; overflow:hidden; border:1px solid var(--border); background:var(--input-bg); transition:all 0.2s; margin-bottom:8px; }}
  ...
  /* Summary */
  .summary-box {{ background:var(--input-bg); border:1px solid var(--border); border-radius:10px;
    padding:1.3rem; line-height:1.85; font-size:0.88rem; }}
  ...
  .video-meta {{ display:flex; gap:1.2rem; flex-wrap:wrap; margin-top:0.4rem; font-size:0.8rem; color:var(--subtext); }}
...
if app_mode == "Video Analysis":
    ...
    with col_frames:
        extract_frames = st.checkbox("Extract Visual Frames", value=False, help="Smart keyframe capture via scene detection")
    ...
    if st.session_state.vid_results:
        ...
        if video_info:
            ...
            st.markdown(f"""<div class="card" style="padding:1rem 1.4rem;">
              <p style="margin:0; font-weight:700; font-size:1rem;">{v_title}</p>
        ...
        with tabs[1]:
            if not segments: st.info("No timeline data.")
                ...
                for seg in segments:
                    h += f"<div style='margin-bottom:10px; padding-bottom:10px; border-bottom:1px solid {t['border']};'><span style='color:{t['accent']}; font-family:\"JetBrains Mono\",monospace; font-size:0.75rem; margin-right:8px;'>[{format_time(seg.get('start',0))}]</span>{seg.get('text','')}</div>"
        ...
        with tabs[3]:
            ...
            if not events: st.info("No events detected.")
                ...
                st.markdown(f"<p style='color:{t['subtext']}; font-size:0.78rem; margin-bottom:10px;'>{len(events)} events detected</p>", unsafe_allow_html=True)
                h = "<div class='custom-scroll'>"
                for ev in events:
                    c = t["negative"] if "Question" in ev["type"] else t["positive"]
                    h += f"<div class='event-box' style='border-left:3px solid {c};'><span style='color:{c}; font-family:\"JetBrains Mono\",monospace; font-size:0.72rem; text-transform:uppercase;'>{ev['time']} · {ev['type']}</span><p style='margin:4px 0 0; font-size:0.86rem;'>{ev['text']}</p></div>"
                h += "</div>"; st.markdown(h, unsafe_allow_html=True)
                ...
                for row_start in range(0, len(keyframes), 3):
                    ...
                    for j in range(3):
                        ...
                        if idx < len(keyframes):
                            ...
                            with cols[j]:
                                # Native st.image has built-in expand/fullscreen on hover
    ...
    if st.session_state.live_transcript:
        ...
        with lt4:
            ...
            if not evts: st.info("No events yet.")
                ...
                for ev in evts:
                    c = t["negative"] if "Question" in ev["type"] else t["positive"]
                    h += f"<div class='event-box' style='border-left:3px solid {c};'><span style='color:{c}; font-family:\"JetBrains Mono\",monospace; font-size:0.72rem; text-transform:uppercase;'>{ev['time']} · {ev['type']}</span><p style='margin:4px 0 0;'>{ev['text']}</p></div>"
                h += "</div>"; st.markdown(h, unsafe_allow_html=True)
        with lt5:
            if segments:
                cfg = {'displayModeBar': False}
...
# FOOTER
st.markdown(f"""<div style="text-align:center; padding:1.8rem 0 0.8rem; font-size:0.65rem; color:var(--subtext); letter-spacing:0.06em;">
  Whisper · yt-dlp · Transformers · Streamlit — Zero paid APIs</div>""", unsafe_allow_html=True)